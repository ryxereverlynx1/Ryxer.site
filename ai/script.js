...
analyzeIntent(input) {
        ...
        if (text.match(/\b(flip a coin|roll a dice|rng|random number)\b/)) return 'action_rng';
...
async generateResponse(input) {
        ...
        if (intent === 'action_theme') {
            ...
            if (lowerInput.includes('dark')) newTheme = 'dark';
            if (lowerInput.includes('light')) newTheme = 'light';
            
            setTheme(newTheme); // Physically changes the website theme!
            reply = `System theme has been updated to **${newTheme.toUpperCase()}** mode.`;
            this.updateMemory(input, reply);
            return reply;
        }
        if (intent === 'action_clear') {
            // Silently wipe the local storage memory without the annoying browser popup
            localStorage.removeItem(getChatHistoryKey());
            this.memory = [];
            
            // Wait 2 seconds, then refresh the chat UI
        ...
        if (intent === 'action_music') {
            toggleMusic(); // Physically clicks the music button
        ...
        if (intent === 'action_rng') {
            if (input.toLowerCase().includes('coin')) {
                const coin = Math.random() > 0.5 ? "Heads" : "Tails";
                reply = `I flipped a digital coin. It landed on **${coin}**.`;
            } else {
                const dice = Math.floor(Math.random() * 6) + 1;
                reply = `I rolled a virtual six-sided dice. It landed on **${dice}**.`;
            }
            this.updateMemory(input, reply);
            return reply;
        }
        ...
        if (intent === 'action_color') {
            ...
            // Generates the hex and injects a live colored square right into the chat
...
// --- 2. LOCAL NEURAL NETWORK MODE (Offline Fallback) ---
        ...
        if (intent === 'fallback') {
            try {
                // 1. Attempt to silently search Wikipedia's Free Open API
                const wikiEndpoint = `https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&exintro&explaintext&generator=search&gsrsearch=${encodeURIComponent(input)}&gsrlimit=1&origin=*`;
                ...
                if (wikiData.query && wikiData.query.pages) {
                    ...
                    if (extract) {
                        // Keep it conversational by grabbing just the first 2-3 sentences
                        const sentences = extract.split('. ').slice(0, 3).join('. ') + (extract.split('. ').length > 3 ? '...' : '.');
                        reply = `${sentences}`;
                        this.updateMemory(input, reply);
                        return reply;
                    }
                ...
                console.warn("External data fetch failed, reverting to standard fallback.", e);
            }

            // 2. If Wikipedia finds absolutely nothing, fall back to the secure Google link
            ...
            // Standard offline intents (Calculator, Theme, etc.)
            const responses = this.knowledgeBase[intent];
            reply = responses[Math.floor(Math.random() * responses.length)];

            // Dynamic Token Injection (Time/Date)
